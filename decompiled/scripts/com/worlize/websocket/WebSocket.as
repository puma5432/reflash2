package com.worlize.websocket
{
   ...
   public class WebSocket extends EventDispatcher
   {
      ...
      private function init() : void
      {
         ...
         this.closeTimer.addEventListener("timer",this.handleCloseTimer);
         this.handshakeTimer = new Timer(this.handshakeTimeout,1);
         this.handshakeTimer.addEventListener("timer",this.handleHandshakeTimer);
         this.socket = this.secure ? new SecureSocket() : new Socket();
         this.socket.endian = "bigEndian";
         this.socket.timeout = this.timeout;
         this.socket.addEventListener("connect",this.handleSocketConnect);
         this.socket.addEventListener("ioError",this.handleSocketIOError);
         this.socket.addEventListener("securityError",this.handleSocketSecurityError);
         this.socket.addEventListener("close",this.handleSocketClose);
         this.socket.addEventListener("socketData",this.handleSocketData);
         this._readyState = 3;
      }
      ...
      {
         if(this._readyState === 3 || this._readyState === 2)
         {
            this._readyState = 0;
            this.generateNonce();
      ...
      public function addBinaryChainBuildingCertificate(param1:ByteArray, param2:Boolean) : void
      {
         ...
         (this.socket as SecureSocket).addBinaryChainBuildingCertificate(param1,param2);
      }
      ...
      private function parseUrl() : void
      {
         ...
         var _loc2_:uint = uint(parseInt(this._uri.port,10));
      ...
      public function get uri() : String
      {
         ...
         _loc1_ += this._host;
         if(Boolean(this._secure) && this._port !== 443 || !this._secure && this._port !== 80)
         {
      ...
      private function fragmentAndSend(param1:WebSocketFrame) : void
      {
         ...
         var _loc7_:uint = this.config.fragmentationThreshold;
         if(Boolean(this.config.fragmentOutgoingMessages) && Boolean(param1.binaryPayload) && param1.binaryPayload.length > _loc7_)
         {
            param1.binaryPayload.position = 0;
            _loc2_ = int(param1.binaryPayload.length);
            _loc3_ = int(Math.ceil(_loc2_ / _loc7_));
            _loc4_ = 1;
            while(_loc4_ <= _loc3_)
            {
               _loc5_ = new WebSocketFrame();
               _loc5_.opcode = _loc4_ === 1 ? param1.opcode : 0;
               _loc5_.fin = _loc4_ === _loc3_;
               _loc6_ = _loc4_ === _loc3_ ? int(_loc2_ - _loc7_ * (_loc4_ - 1)) : int(_loc7_);
               param1.binaryPayload.position = _loc7_ * (_loc4_ - 1);
               _loc5_.binaryPayload = new ByteArray();
               param1.binaryPayload.readBytes(_loc5_.binaryPayload,0,_loc6_);
               this.sendFrame(_loc5_);
               _loc4_++;
            }
         }
         else
      ...
      public function close(param1:Boolean = true) : void
      {
         ...
         var _loc2_:* = null;
         var _loc3_:* = null;
         if(!this.socket.connected && this._readyState === 0)
         {
            this._readyState = 2;
            try
            {
      ...
      private function handleSocketData(param1:ProgressEvent = null) : void
      {
         ...
         while(Boolean(this.socket.connected) && Boolean(this.currentFrame.addData(this.socket,this.fragmentationOpcode,this.config)) && !this.fatalError)
         {
            if(this.currentFrame.protocolError)
            {
               this.drop(1002,this.currentFrame.dropReason);
               return;
            }
            if(this.currentFrame.frameTooLarge)
            {
               this.drop(1009,this.currentFrame.dropReason);
               return;
            }
      ...
      private function inflate(param1:ByteArray) : void
      {
         param1.position = param1.length;
         param1.writeUnsignedInt(65535);
         param1[0] |= 1;
         param1.inflate();
      }
      ...
      private function processFrame(param1:WebSocketFrame) : void
      {
         ...
         var _loc8_:* = null;
         var _loc9_:* = null;
         if(param1.rsv1 && !this.serverSupportsDeflate)
         {
            this.drop(1002,"Received frame with rsv1 set without permessage-deflate negotiated.");
         }
         if(param1.rsv2 || param1.rsv3)
         {
            this.drop(1002,"Received frame with reserved bit set without a negotiated extension.");
            return;
         }
         switch(param1.opcode)
         {
            case 2:
               if(this.config.assembleFragments)
               {
                  ...
                  {
                     this.drop(1002,"Illegal BINARY_FRAME received in the middle of a fragmented message.  Expected a continuation or control frame.");
                     return;
                  }
                  if(param1.fin)
                  {
                     _loc2_ = new WebSocketEvent("message");
                     _loc2_.message = new WebSocketMessage();
                     _loc2_.message.type = "binary";
                     if(param1.rsv1)
                     {
                        this.inflate(param1.binaryPayload);
                     }
            ...
            case 1:
               if(this.config.assembleFragments)
               {
                  if(this.frameQueue.length !== 0)
                  {
                     this.drop(1002,"Illegal TEXT_FRAME received in the middle of a fragmented message.  Expected a continuation or control frame.");
                     return;
                  }
                  if(param1.fin)
                  {
                     _loc2_ = new WebSocketEvent("message");
                     _loc2_.message = new WebSocketMessage();
                     _loc2_.message.type = "utf8";
                     if(param1.rsv1)
                     {
                        this.inflate(param1.binaryPayload);
                     }
                     _loc2_.message.utf8Data = param1.binaryPayload.readMultiByte(param1.binaryPayload.length,"utf-8");
                     dispatchEvent(_loc2_);
                  }
                  else
                  {
            ...
            case 0:
               if(this.config.assembleFragments)
               {
                  if(this.fragmentationOpcode === 0 && param1.opcode === 0)
                  {
                     this.drop(1002,"Unexpected continuation frame.");
                     return;
                  }
                  this.fragmentationSize += param1.length;
                  if(this.fragmentationSize > this.config.maxMessageSize)
                  {
                     this.drop(1009,"Maximum message size exceeded.");
                     return;
                  }
                  this.frameQueue.push(param1);
                  if(param1.fin)
                  {
                     _loc2_ = new WebSocketEvent("message");
                     _loc2_.message = new WebSocketMessage();
                     _loc5_ = int(this.frameQueue[0].opcode);
                     _loc6_ = new ByteArray();
                     _loc7_ = 0;
                     _loc3_ = 0;
                     while(_loc3_ < this.frameQueue.length)
                     {
                        _loc7_ += this.frameQueue[_loc3_].length;
                        _loc3_++;
                     }
                     if(_loc7_ > this.config.maxMessageSize)
                     {
                        this.drop(1009,"Message size of " + _loc7_ + " bytes exceeds maximum accepted message size of " + this.config.maxMessageSize + " bytes.");
                        return;
                     }
                     _loc3_ = 0;
                     while(_loc3_ < this.frameQueue.length)
                     {
                        ...
                        _loc6_.writeBytes(_loc4_.binaryPayload,0,_loc4_.binaryPayload.length);
                        _loc4_.binaryPayload.clear();
                        _loc3_++;
                     }
                     if(this.frameQueue[0].rsv1)
                     {
                        this.inflate(_loc6_);
                     }
                     _loc6_.position = 0;
                     switch(_loc5_)
                     {
                        case 2:
                           _loc2_.message.type = "binary";
                           _loc2_.message.binaryData = _loc6_;
                           break;
                        case 1:
                           _loc2_.message.type = "utf8";
                           _loc2_.message.utf8Data = _loc6_.readMultiByte(_loc6_.length,"utf-8");
                           break;
                        default:
                           this.drop(1002,"Unexpected first opcode in fragmentation sequence: 0x" + _loc5_.toString(16));
                           return;
                     }
                     this.frameQueue = new Vector.<WebSocketFrame>();
                     this.fragmentationOpcode = 0;
            ...
            case 9:
               ...
               _loc8_ = new WebSocketEvent("ping",false,true);
               _loc8_.frame = param1;
            ...
            case 10:
               ...
               _loc9_ = new WebSocketEvent("pong");
               _loc9_.frame = param1;
            ...
            default:
               ...
               this.drop(1002,"Unrecognized Opcode: 0x" + param1.opcode.toString(16));
               return;
         }
      }
      ...
      private function sendHandshake() : void
      {
         ...
         var _loc2_:String = this.host;
         if(Boolean(this._secure) && this._port !== 443 || !this._secure && this._port !== 80)
         {
            _loc2_ += ":" + this._port.toString();
         }
         var _loc3_:String = "";
         _loc3_ += "GET " + this.resource + " HTTP/1.1\r\n";
         _loc3_ += "Host: " + _loc2_ + "\r\n";
         _loc3_ += "Upgrade: websocket\r\n";
         _loc3_ += "Connection: Upgrade\r\n";
         _loc3_ += "Sec-WebSocket-Key: " + this.base64nonce + "\r\n";
         if(this._origin)
         {
            _loc3_ += "Origin: " + this._origin + "\r\n";
         }
         _loc3_ += "Sec-WebSocket-Version: 13\r\n";
         if(this._protocols)
         {
            _loc1_ = this._protocols.join(", ");
            _loc3_ += "Sec-WebSocket-Protocol: " + _loc1_ + "\r\n";
         }
         _loc3_ += "Sec-WebSocket-Extensions: permessage-deflate; server_no_context_takeover\r\n";
         _loc3_ += "\r\n";
         if(this.debug)
         {
            logger(_loc3_);
         }
         this.socket.writeMultiByte(_loc3_,"us-ascii");
      ...
      private function readServerHandshake() : void
      {
         ...
         var lcName:* = null;
         var lcValue:* = null;
         var extensionsThisLine:* = null;
         var byteArray:* = null;
         var expectedKey:* = null;
         var headersTerminatorIndex:int = -1;
         while(headersTerminatorIndex === -1 && Boolean(this.readHandshakeLine()))
         {
            if(this.handshakeBytesReceived > 10240)
            {
               this.failHandshake("Received more than 10240 bytes during handshake.");
               return;
            }
         ...
         statusCode = int(parseInt(responseLineMatch[2],10));
         ...
         this.serverSupportsDeflate = false;
         for each(ext in this.serverExtensions)
         {
            if(ext.indexOf("permessage-deflate") != -1)
            {
               this.serverSupportsDeflate = true;
            }
         }
         this.handshakeTimer.stop();
         this.handshakeTimer.reset();
