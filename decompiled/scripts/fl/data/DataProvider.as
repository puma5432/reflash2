package fl.data
{
   ...
   public class DataProvider extends EventDispatcher
   {
      ...
      public function merge(param1:Object) : void
      {
         ...
         var _loc5_:uint = this.data.length;
         this.dispatchPreChangeEvent(DataChangeType.ADD,this.data.slice(_loc5_,this.data.length),_loc5_,this.data.length - 1);
         while(_loc6_ < _loc4_)
         {
            _loc2_ = _loc3_[_loc6_];
            if(this.getItemIndex(_loc2_) == -1)
            {
               this.data.push(_loc2_);
            }
            _loc6_++;
         }
         if(this.data.length > _loc5_)
         {
            this.dispatchChangeEvent(DataChangeType.ADD,this.data.slice(_loc5_,this.data.length),_loc5_,this.data.length - 1);
         }
         else
         {
