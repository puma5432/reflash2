package fl.controls
{
   ...
   public class SelectableList extends BaseScrollPane implements IFocusManagerComponent
   {
      ...
      public function set selectedItems(param1:Array) : void
      {
         ...
         {
            _loc2_ = this._dataProvider.getItemIndex(param1[_loc4_]);
            if(_loc2_ != -1)
            {
               _loc3_.push(_loc2_);
            }
      ...
      public function getNextIndexAtLetter(param1:String, param2:int = -1) : int
      {
         ...
         var _loc6_:int = this.length - 1;
         var _loc7_:Number = 0;
         while(_loc7_ < _loc6_)
         {
            _loc3_ = param2 + 1 + _loc7_;
            if(_loc3_ > this.length - 1)
            {
               _loc3_ -= this.length;
            }
            ...
            _loc5_ = this.itemToLabel(_loc4_);
            if(_loc5_ != null)
            {
               if(_loc5_.charAt(0).toUpperCase() == param1)
               {
                  return _loc3_;
               }
            }
      ...
      {
         var _loc2_:uint = 0;
         var _loc3_:int = int(param1.startIndex);
         var _loc4_:int = int(param1.endIndex);
         var _loc5_:String = param1.changeType;
         if(_loc5_ == DataChangeType.INVALIDATE_ALL)
         {
            this.clearSelection();
            this.invalidateList();
         }
         else if(_loc5_ == DataChangeType.INVALIDATE)
         {
            _loc2_ = 0;
            while(_loc2_ < param1.items.length)
            {
               this.invalidateItem(param1.items[_loc2_]);
               _loc2_++;
            }
         }
         else if(_loc5_ == DataChangeType.ADD)
         {
            _loc2_ = 0;
            while(_loc2_ < this._selectedIndices.length)
            {
               if(this._selectedIndices[_loc2_] >= _loc3_)
               {
                  this._selectedIndices[_loc2_] += _loc3_ - _loc4_;
               }
               _loc2_++;
            }
         }
         else if(_loc5_ == DataChangeType.REMOVE)
         {
            _loc2_ = 0;
            while(_loc2_ < this._selectedIndices.length)
            {
               if(this._selectedIndices[_loc2_] >= _loc3_)
               {
                  if(this._selectedIndices[_loc2_] <= _loc4_)
                  {
                     delete this._selectedIndices[_loc2_];
                  }
                  else
                  {
                     this._selectedIndices[_loc2_] -= _loc3_ - _loc4_ + 1;
                  }
               }
               _loc2_++;
            }
         }
         else if(_loc5_ == DataChangeType.REMOVE_ALL)
         {
            this.clearSelection();
         }
         else if(_loc5_ != DataChangeType.REPLACE)
         {
      ...
      protected function handleCellRendererClick(param1:MouseEvent) : void
      {
         ...
         var _loc5_:uint = _loc4_.listData.index;
         if(!dispatchEvent(new ListEvent(ListEvent.ITEM_CLICK,false,true,_loc4_.listData.column,_loc4_.listData.row,_loc5_,_loc4_.data)) || !this._selectable)
         {
            return;
         }
         var _loc6_:int = this.selectedIndices.indexOf(_loc5_);
         if(!this._allowMultipleSelection)
         {
            ...
            this.lastCaretIndex = this.caretIndex = _loc5_;
         }
         else if(param1.shiftKey)
         {
            _loc3_ = this._selectedIndices.length > 0 ? uint(this._selectedIndices[0]) : _loc5_;
            this._selectedIndices = [];
            if(_loc3_ > _loc5_)
            {
               _loc2_ = int(_loc3_);
               while(_loc2_ >= _loc5_)
               {
                  this._selectedIndices.push(_loc2_);
                  _loc2_--;
               }
            }
            else
         ...
         else if(param1.ctrlKey)
         {
            if(_loc6_ != -1)
            {
               ...
               this._selectedIndices.splice(_loc6_,1);
            }
            else
            {
               _loc4_.selected = true;
      ...
      protected function updateRendererStyles() : void
      {
         ...
         var _loc4_:uint = 0;
         var _loc2_:Array = this.availableCellRenderers.concat(this.activeCellRenderers);
         var _loc3_:uint = _loc2_.length;
         while(_loc4_ < _loc3_)
         {
            if(_loc2_[_loc4_].setStyle != null)
            {
               for(_loc1_ in this.updatedRendererStyles)
               {
                  _loc2_[_loc4_].setStyle(_loc1_,this.updatedRendererStyles[_loc1_]);
               }
               _loc2_[_loc4_].drawNow();
            }
            _loc4_++;
         }
         this.updatedRendererStyles = {};
