...
def _resolve_payload() -> Path:
    ...
    if not prep.exists():
        raise LaunchError(f"Launch payload not found at {payload} and preparation script is missing ({prep}).")
    proc = subprocess.run(["bash", str(prep)], cwd=_REPO_ROOT, capture_output=True, text=True)
    if proc.returncode or not app_xml.exists():
        raise LaunchError(f"Payload preparation failed (exit {proc.returncode}).\n{(proc.stderr or proc.stdout or '').strip()[-2000:]}")
    return payload
...
def ensure_game_running(host: str = "127.0.0.1", port: int = DEFAULT_PORT, timeout: float = 90., air_sdk_home: Optional[str] = None, verbose: bool = True) -> bool:
    ...
    with _lock:
        ...
        if _spawned is not None and _spawned.poll() is not None:
            _spawned = None
        sdk, payload = _resolve_air_sdk(air_sdk_home), _resolve_payload();
        app_xml = payload / "META-INF" / "AIR" / "application.xml"
        log_dir = _REPO_ROOT / ".macos";
        log_dir.mkdir(exist_ok=True);
        _log_path = log_dir / "adl.log"
        if verbose:
            print(f"[ssf2_rl] Launching SSF2 via {sdk / 'bin' / 'adl'} (log: {_log_path}) ...")
        with open(_log_path, "ab") as log:
            _spawned = subprocess.Popen([str(sdk / "bin" / "adl"), str(app_xml), str(payload)], cwd=payload, stdout=log, stderr=subprocess.STDOUT, start_new_session=True)
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        if port_open(host, port):
            return False
        if _spawned is not None and _spawned.poll() is not None:
            raise LaunchError(f"Game process exited early (code {_spawned.returncode}). See {_log_path}.")
        time.sleep(.5)
