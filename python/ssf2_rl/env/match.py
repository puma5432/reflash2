...
def reset_match(env: "SSF2Env") -> dict:
    ...
    state = reply_state(bridge.wait_reply(request, "match_ready", timeout=30.0), "restart")
    for player_id, declaration in env.players.items():
        if isinstance(declaration, Human):
            continue
        bridge.takeover(player_id)
        if isinstance(declaration, Bot):
            bridge.send_input(player_id, 0);
            declaration.reset()
