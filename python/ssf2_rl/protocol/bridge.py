...
class SSF2Bridge:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = DEFAULT_PORT, timeout: float = 10.0, state_transport: str = "json") -> None:
        ...
        self._sock: Optional[socket.socket] = None; self._reader: Optional[threading.Thread] = None
        self._stop = threading.Event(); self._reader_error: Optional[str] = None; self._send_lock = threading.Lock()
        self._states: queue.Queue[dict[str, Any]] = queue.Queue(maxsize=64); self._latest: Optional[dict[str, Any]] = None
        self._latest_lock = threading.Lock(); self._events: queue.Queue[dict[str, Any]] = queue.Queue(); self._deferred_events: deque[dict[str, Any]] = deque()
        self._request_lock = threading.Lock(); self._next_request = 1; self._char_names: dict[int, str] = {}
        self.match_generation = 0; self.match_metadata: dict[str, Any] = {}; self.hello: Optional[dict[str, Any]] = None

    def connect(self) -> dict[str, Any]:
        if self._sock is not None: raise BridgeError("already connected")
        self._sock = socket.create_connection((self._host, self._port), self._timeout); self._sock.settimeout(None); self._stop.clear(); self._reader_error = None
        hello = self._next_message(self._timeout)
        if hello is None or hello.get("type") != "hello":
            self.close(); raise BridgeError(f"expected hello, got {hello!r}")
        self.hello = hello; self._reader = threading.Thread(target=self._read_loop, daemon=True, name="ssf2-bridge-reader"); self._reader.start()
        if self.state_transport not in set(hello.get("stateTransports", ["json"])): self.state_transport = "json"
    ...
    def close(self) -> None:
        ...
        if self._sock:
            ...
            self._sock = None
        if self._reader: self._reader.join(timeout=2); self._reader = None

    def __enter__(self) -> "SSF2Bridge": self.connect(); return self
        ...
        while not self._stop.is_set():
            ...
            while buffer:
                if buffer.startswith(BINARY_MAGIC):
                    if len(buffer) < BINARY_HEADER.size: break
                    _, kind, schema, _, length = BINARY_HEADER.unpack_from(buffer)
                    if schema != 3 or length > MAX_BINARY_PAYLOAD: return self._protocol_error("invalid binary state header")
                    if len(buffer) < BINARY_HEADER.size + length: break
                    payload = bytes(buffer[BINARY_HEADER.size:BINARY_HEADER.size + length]); del buffer[:BINARY_HEADER.size + length]
                    try:
                        event, generation = decode_binary_state(kind, payload, self._char_names); self.match_generation = generation; self._dispatch(event)
                    except ValueError as exc: return self._protocol_error(str(exc))
                    continue
                if buffer.startswith(EPISODE_MAGIC):
                    if len(buffer) < EPISODE_HEADER.size: break
                    _, generation, count, length = EPISODE_HEADER.unpack_from(buffer)
                    if length > MAX_EPISODE_PAYLOAD: return self._protocol_error("invalid episode header")
                    if len(buffer) < EPISODE_HEADER.size + length: break
                    payload = bytes(buffer[EPISODE_HEADER.size:EPISODE_HEADER.size + length]); del buffer[:EPISODE_HEADER.size + length]
                    try: self._events.put(decode_episode(generation, count, payload, self._char_names))
                    except ValueError as exc: return self._protocol_error(str(exc))
                    continue
                ...
                try: self._dispatch(json.loads(line.decode("utf-8")))
                except (UnicodeDecodeError, json.JSONDecodeError) as exc: return self._protocol_error(str(exc))
        if not self._stop.is_set() and self._reader_error is None: self._reader_error = "bridge disconnected"
        self._events.put({"type": "disconnected"})

    def _protocol_error(self, message: str) -> None:
        self._reader_error = message; self._events.put({"type": "protocol_error", "message": message})

    def _dispatch(self, message: dict[str, Any]) -> None:
        state = message.get("state") if isinstance(message.get("state"), dict) else message
        if isinstance(state, dict):
            for character in state.get("chars", []):
                if "id" in character and character.get("name"): self._char_names[int(character["id"])] = str(character["name"])
        if message.get("generation") is not None: self.match_generation = int(message["generation"])
        if isinstance(message.get("metadata"), dict): self.match_metadata = message["metadata"]
        if message.get("type") == "match_end": self.match_metadata = {}; self._char_names.clear()
        if message.get("type") == "state":
            with self._latest_lock: self._latest = message
            try: self._states.put_nowait(message)
            except queue.Full:
                try: self._states.get_nowait(); self._states.put_nowait(message)
                except queue.Empty: pass
        else: self._events.put(message)

    def _next_message(self, timeout: float) -> Optional[dict[str, Any]]:
        assert self._sock is not None; self._sock.settimeout(timeout); buffer = b""
        try:
            while b"\n" not in buffer:
                chunk = self._sock.recv(4096)
                if not chunk: return None
                buffer += chunk
        finally: self._sock.settimeout(None)
        return json.loads(buffer.split(b"\n", 1)[0].decode("utf-8"))

    def latest_state(self) -> Optional[dict[str, Any]]:
        with self._latest_lock: return self._latest

    def wait_state(self, timeout: float = 5.0, min_frame: Optional[int] = None) -> dict[str, Any]:
        deadline = time.monotonic() + timeout
        while True:
            if self._reader_error: raise BridgeError(self._reader_error)
            remaining = deadline - time.monotonic()
            if remaining <= 0: raise BridgeError("timed out waiting for state")
            try: state = self._states.get(timeout=min(remaining, .25))
            except queue.Empty: continue
            if min_frame is None or state.get("frame", 0) >= min_frame: return state
    ...
    def wait_reply(self, request: int, expected_type: str, timeout: float = 5.0) -> dict[str, Any]:
        ...
        while True:
            for event in list(self._deferred_events):
                ...
                if event.get("request") != request: continue
                self._deferred_events.remove(event)
                if event.get("type") == "error": raise BridgeError(event.get("message", "game rejected command"))
                if event.get("type") == expected_type: return event
                raise BridgeError(f"expected {expected_type!r} for request {request}, got {event.get('type')!r}")
            remaining = deadline - time.monotonic()
            if remaining <= 0: raise BridgeError(f"timed out waiting for {expected_type!r} reply to request {request}")
            try: self._deferred_events.append(self._events.get(timeout=min(remaining, .25)))
            except queue.Empty: continue
    ...
    def wait_episode(self, timeout: float = 30.0) -> dict[str, Any]:
        ...
        while True:
            for event in list(self._deferred_events):
                self._deferred_events.remove(event)
                if event.get("type") == "episode": return event
                if event.get("type") in {"disconnected", "protocol_error"}: raise BridgeError(event.get("message", "bridge disconnected while waiting for episode"))
            remaining = deadline - time.monotonic()
            if remaining <= 0: raise BridgeError("timed out waiting for episode")
            try: self._deferred_events.append(self._events.get(timeout=min(remaining, .25)))
            except queue.Empty: continue

    def _send(self, message: dict[str, Any]) -> None:
        if self._sock is None: raise BridgeError("not connected")
        with self._send_lock: self._sock.sendall((json.dumps(message, separators=(",", ":")) + "\n").encode())

    def _send_request(self, command: str) -> int:
        with self._request_lock: request = self._next_request; self._next_request += 1
        self._send({"type": command, "request": request}); return request
